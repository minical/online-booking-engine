...
$(".configure-new-channel").on("click", function() {
    ...
    $.ajax({
        type: "POST",
        url: getBaseURL() + "integrations/configure_roomsy_channel_manager_AJAX",
        data: {
            data: data
        },
...
$('.manage-channel').on("click", function() {
    var otaId = $(this).data('ota_id');
    window.location.href = getBaseURL() + "integrations/roomsy_channel_manager/manage/"+otaId;
});
...
$(".deconfigure-channel").on("click", function() {
    ...
    $.ajax({
        type: "POST",
        url: getBaseURL() + "integrations/deconfigure_roomsy_channel_manager_AJAX",
        data: {
...
$(".login-myallocator").on("click", function() {
    ...
    $.ajax({
        type: "POST",
        url: getBaseURL() + "integrations/myallocator_login_AJAX",
        data: {
            data: data
        },
        dataType: 'json',
        success: function (response) {
            ...
            if(response.user_status == 'all_set'){
                window.location.href = getBaseURL() + "integrations/myallocator/manage/"+otaId;
            }
...
$('.property-mapping-myallocator').click(function(){
    ...
    $.ajax({
         type   : "POST",
         url    : getBaseURL() + "integrations/myallocator_property_mapping_AJAX",
         data   : {
...
$('.save_myallocator_mapping').on('submit', function(e){
    ...
    $.ajax({
	    url    : getBaseURL() + "integrations/save_myallocator_mapping_AJAX",
	    type   : "POST",
        dataType: "json",
...
$('.manage-agoda-channel').on("click", function() {
    var otaId = $(this).data('ota_id');
    window.location.href = getBaseURL() + "integrations/agoda/manage/"+otaId;
});

$(function(){
    if($(".integrations").length > 0 && $(".integrations").attr('integrations_enabled') != 1){
        //alert("OTA integration is available for our Premium subscribers. \n\nIf you are interested in integrating your OTA(Expedia, Booking.com etc.) rates and reservations with Roomsy contact us at support@minical.io");
    }
